var superLocal = (function() {
    ...
    const superLocal = {
        ...
        findNewest: function() {
            ...
            for (let i = 0; i < totalKeys; i++) {
                if (this.prefixRegex().test(localKeys[i]) && this.modifiedRegex().test(localKeys[i])) {
                    const currentKey = localKeys[i].replace(this.settings.dbPrefix + this.settings.separator, "")
                        .replace(this.settings.separator + "modified", "");
                    const thisModified = localStorage[this.settings.dbPrefix +
        ...
        findSmallest: function() {
            ...
            for (let i = 0; i < totalKeys; i++) {
                if (this.prefixRegex().test(localKeys[i]) && this.datafieldRegex().test(localKeys[i])) {
                    const currentKey = localKeys[i].replace(this.settings.dbPrefix + this.settings.separator, "")
                        .replace(this.settings.separator + "modified", "");
                    const thisData = localStorage[this.settings.dbPrefix +
                        this.settings.separator +
                        currentKey +
                        this.settings.separator +
                        this.settings.dataFieldName].length;
        ...
        fetch: function(param) {
            ...
            const recordPrefix = this.settings.dbPrefix + this.settings.separator + uID + this.settings.separator;
            data.uID = uID;
        ...
        listAllCreated: function() {
            ...
            for (let i = 0; i < totalKeys; i++) {
                if (this.prefixRegex().test(localKeys[i]) && this.createdRegex().test(localKeys[i])) {
                    const currentKey = localKeys[i].replace(this.settings.dbPrefix + this.settings.separator, "")
                        .replace(this.settings.separator + "created", "");
                    allKeys[currentKey] = localStorage[this.settings.dbPrefix +
                        this.settings.separator +
                        currentKey +
                        this.settings.separator +
                        "created"];
